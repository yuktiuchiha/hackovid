...
<meta name="viewport" content="width=device-width, initial-scale=1">
 ...
 <?php
 	$sql1=mysqli_query($db,"SELECT register.pic,queries.username from register inner join queries on register.username=queries.username and register.status='student' group by username order by seen_status;");
 ?>
 <!-----   left box   --------->
 	...
 	<div class="left_box2">
 		...
 		<div class="list">
 			<?php
 				...
 				{
 					echo "<tr>";
 						echo "<td>"; echo "<img class='img-circle profile_img' height=60 width=60 src='images/".$res1['pic']."'> ";
 						echo "</td>";
...
/*--------------- if submit1 is pressed --------*/
 			...
 			if(isset($_POST['submit1']))
 			{
	 			...
	 			$res=mysqli_query($db,"SELECT * from `queries` where username='$_SESSION[login_user]' or receiver='$_POST[username]' ;");
	 			mysqli_query($db,"UPDATE queries set seen_status='yes' where sender='student' and username='$_POST[username]' ;");
			...
			<!------   student    -------->
	 		...
	 		<?php
 					...
 					?>
 						<center>
 						<img src="images/tenor.gif" style="margin-top: 35px; height: 80%; width: 90%; border-radius: 50%;"></center>
 					<?php	
	 		...
	 		else
	 		{
	 			...
	 			{
	 				mysqli_query($db,"INSERT into `hackovid`.`queries` values('', '$_SESSION[login_user]', '$_POST[message]', 'no', 'admin', '$_SESSION[username]' );");
	 				$res=mysqli_query($db,"SELECT * FROM `queries` WHERE username='$_SESSION[login_user]' AND receiver='$_SESSION[username]' or username='$_SESSION[username]' ;");
